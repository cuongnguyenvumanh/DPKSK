/**
 * MWK - CHI TIẾT LỊCH PHƯỜNG | LICH-PHUONG MODULE DETAIL JS LOGIC
 */
...
function initDetailModule() {
    ...
    if (item) {
        ...
        if (titleElem) titleElem.innerText = item.maLich || '260813 - LP - S,C';
        ...
        if (khungGioElem) {
            let timeStr = '07:30 - 11:00, 13:30 - 17:30';
        ...
        if (diaDiemElem) diaDiemElem.innerText = item.diaDiemKham || 'Trạm Y tế Phường Cống Vị';

        const soKhachElem = document.getElementById('detail-so-khach');
        if (soKhachElem) soKhachElem.innerText = `${item.soLuongKhach || item.guests || 120} KH`;

        // Render Danh mục khám Table
        const tableDM = document.getElementById('detail-table-danh-muc');
        if (tableDM) {
            const categories = item.danhMucKham || [
                { name: 'Khám Nội tổng quát tại phường', guests: 120 },
                { name: 'Lấy máu xét nghiệm cộng đồng', guests: 120 }
            ];
            let html = '';
            categories.forEach((cat, idx) => {
                html += `
                    <tr>
                        ...
                        <td class="py-2 px-3 text-right font-bold text-[#7E22CE]">${cat.guests || 120}</td>
                    </tr>
                `;
            });
        ...
        if (tableNS) {
            const staff = item.nhanSu || [
                { title: 'Bác sĩ Phụ trách Lịch phường', count: 3 },
                { title: 'Điều dưỡng Trạm y tế', count: 4 }
            ];
            let html = '';
            staff.forEach((st, idx) => {
                html += `
                    <tr>
                        ...
                        <td class="py-2 px-3 text-right font-bold text-[#7E22CE]">${st.count || 2}</td>
                    </tr>
                `;
            });
