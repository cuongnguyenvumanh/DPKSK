/**
 * MWK - THÊM LỊCH PHƯỜNG | LICH-PHUONG MODULE JS LOGIC
 */
...
function getMasterCategoriesFromStore() {
    ...
    return [
        'Khám Nội tổng quát tại phường',
        'Lấy máu xét nghiệm cộng đồng',
        'Đo Huyết áp & Nhịp tim',
        'Siêu âm ổ bụng tổng quát (Lưu động)',
        'Đo Điện tâm đồ (ECG)',
        'Tư vấn sức khỏe & Cấp phát thuốc',
        'Khám Mắt & Đo thị lực',
        'Khám Tai Mũi Họng'
    ];
}
...
function customizeBreadcrumbForLichPhuong() {
    ...
    breadcrumbContainer.innerHTML = `
        ...
        <i class="fa-solid fa-chevron-right text-[9px] text-[#9CA3AF]"></i>
        <span id="breadcrumb-current" class="font-semibold text-[#1F2937]">Thêm lịch phường</span>
    `;
}
...
function initBreadcrumbWithRetry() {
    ...
    const interval = setInterval(() => {
        ...
        if (container) {
            customizeBreadcrumbForLichPhuong();
...
function handleBreadcrumbClick(targetUrl) {
    if (isFormDirty) {
        if (confirm('Bạn có sự thay đổi chưa lưu tại màn Thêm lịch phường. Bạn có chắc chắn muốn quay lại không?')) {
...
function handleHomeBreadcrumbClick(event) {
    if (isFormDirty) {
        if (!confirm('Bạn có sự thay đổi chưa lưu tại màn Thêm lịch phường. Bạn có chắc chắn muốn quay lại Trang chủ không?')) {
...
function initHeaderDataFromUrl() {
    const params = new URLSearchParams(window.location.search);
    const customerName = params.get('customerName') || 'UBND Phường Cống Vị';
    const customerCode = params.get('customerCode') || 'KAD019962025';
    const estimatedCount = params.get('estimatedCount') || '120';
    ...
    if (facInput && !facInput.value) {
        facInput.value = 'Trạm Y tế Phường Cống Vị';
    }
}
...
function toggleWorkTypePill(btnElem) {
    ...
    if (isSelected) {
        btnElem.classList.remove('active', 'border-[#7E22CE]', 'bg-[#F3E8FF]', 'text-[#7E22CE]', 'font-semibold');
        btnElem.classList.add('border-[#D9DEE5]', 'bg-white', 'text-[#4B5563]');
        if (icon) {
            icon.className = 'fa-solid fa-plus text-[10px] mr-1 text-[#6B7280]';
        }
    } else {
        btnElem.classList.add('active', 'border-[#7E22CE]', 'bg-[#F3E8FF]', 'text-[#7E22CE]', 'font-semibold');
        btnElem.classList.remove('border-[#D9DEE5]', 'bg-white', 'text-[#4B5563]');
        if (icon) {
            icon.className = 'fa-solid fa-check text-[10px] mr-1 text-[#7E22CE]';
        }
    }
}
...
function confirmAddCustomWorkType() {
    ...
    if (pillsContainer && customWrapper) {
        ...
        newBtn.className = 'pill-btn active border-[#7E22CE] bg-[#F3E8FF] text-[#7E22CE] font-semibold';
        newBtn.innerHTML = `<i class="fa-solid fa-check text-[10px] mr-1 text-[#7E22CE]"></i>${val}`;
...
function getInitialCategories() {
    ...
    return [
        { name: categories[0] || 'Khám Nội tổng quát tại phường', guests: 120, sang: true, chieu: true },
        { name: categories[1] || 'Lấy máu xét nghiệm cộng đồng', guests: 120, sang: true, chieu: false },
        { name: categories[2] || 'Đo Huyết áp & Nhịp tim', guests: 120, sang: true, chieu: true },
        { name: categories[3] || 'Tư vấn sức khỏe & Cấp phát thuốc', guests: 120, sang: true, chieu: true }
    ];
}
...
function getInitialStaff() {
    return [
        ...
        { title: 'Điều dưỡng Trạm y tế & Lưu động', count: 4, sang: true, chieu: false },
        { title: 'Cán bộ Y tế / UBND Phường hỗ trợ', count: 3, sang: true, chieu: true },
        { title: 'Điều phối viên KSK', count: 2, sang: true, chieu: true }
    ];
}
...
function addCategoryRow() {
    markFormDirty();
    const guestInput = document.getElementById('guestCount');
    const defaultGuests = guestInput ? parseInt(guestInput.value) || 120 : 120;
    const categories = getMasterCategoriesFromStore();

    categoryRows.push({
        name: categories[0] || 'Danh mục Lịch phường mới',
...
function addStaffRow() {
    markFormDirty();
    staffRows.push({
        title: 'Nhân sự Phường hỗ trợ mới',
        count: 2,
        sang: true,
        chieu: true
    });
...
function handleSaveAndExit() {
    ...
    if (window.showToast) {
        window.showToast('Đã lưu thành công lịch khám phường!', 'success');
    }
...
function handleSaveAndContinue() {
    ...
    const facility = document.getElementById('facility')?.value || 'Trạm Y tế Phường';
    const guestCount = parseInt(document.getElementById('guestCount')?.value) || 120;
    const totalStaff = staffRows.reduce((sum, r) => sum + (parseInt(r.count) || 0), 0);
    ...
    const maLich = `${dateCode} - LP - ${shiftCode}`;

    if (!draft.step2Data.lichPhuong) draft.step2Data.lichPhuong = [];
    const itemIndex = draft.step2Data.lichPhuong.length + 1;
    const newLichPhuongItem = {
        id: `LP-${dateCode}-00${itemIndex}`,
        maLich: maLich,
        loaiLich: 'LICH_PHUONG',
    ...
    draft.step2Data.lichPhuong.push(newLichPhuongItem);
    draft.currentStep = 2;
    ...
    if (window.showToast) {
        window.showToast('Đã lưu lịch khám phường!', 'success');
    }
...
function saveScheduleToDataStore() {
    ...
    draft.step2Data.lichPhuong.push(newItem);
    sessionStorage.setItem(storageKey, JSON.stringify(draft));
}

function openDinhMucModalLP() {
    const modal = document.getElementById('modal-view-dinh-muc-lp');
    if (modal) modal.classList.remove('hidden');
}

function closeDinhMucModalLP() {
    const modal = document.getElementById('modal-view-dinh-muc-lp');
