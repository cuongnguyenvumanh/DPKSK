/**
 * MWK - CHỈNH SỬA LỊCH NGOẠI VIỆN | NGOAI-VIEN MODULE EDIT JS LOGIC
 */
...
function customizeBreadcrumb() {
    ...
    breadcrumbContainer.innerHTML = `
        ...
        <i class="fa-solid fa-chevron-right text-[9px] text-[#9CA3AF]"></i>
        <span class="font-semibold text-[#1F2937]">Chỉnh sửa lịch ngoại viện</span>
    `;
}
...
function loadHeaderDataFromDraft() {
    ...
    const estimatedCount = draft.estimatedCount || '180';
...
function renderCategoryTable() {
    if (categoryRows.length === 0) {
        categoryRows = [
            { name: 'Khám Nội tổng quát ngoại viện', guests: 180, sang: true, chieu: true },
            { name: 'Lấy máu xét nghiệm tại đơn vị', guests: 180, sang: true, chieu: false }
        ];
    }
...
function addCategoryRow() {
    markFormDirty();
    categoryRows.push({ name: 'Danh mục ngoại viện mới', guests: 180, sang: true, chieu: true });
    renderCategoryTable();
}
...
function renderStaffTable() {
    if (staffRows.length === 0) {
        staffRows = [
            ...
            { title: 'Điều dưỡng Lấy mẫu ngoại viện', count: 6, sang: true, chieu: false }
        ];
    }
