/**
 * MWK - CHI TIẾT LỊCH NGOẠI VIỆN | NGOAI-VIEN MODULE DETAIL JS LOGIC
 */
...
function initDetailModule() {
    ...
    if (item) {
        ...
        if (titleElem) titleElem.innerText = item.maLich || '260813 - NV - S';
        ...
        if (diaDiemElem) diaDiemElem.innerText = item.diaDiemKham || 'Khám Ngoại viện';

        const soKhachElem = document.getElementById('detail-so-khach');
        if (soKhachElem) soKhachElem.innerText = `${item.soLuongKhach || item.guests || 180} KH`;

        // Render Danh mục khám Table
        const tableDM = document.getElementById('detail-table-danh-muc');
        if (tableDM) {
            const categories = item.danhMucKham || [
                { name: 'Khám Nội tổng quát ngoại viện', guests: 180 },
                { name: 'Lấy máu xét nghiệm tại đơn vị', guests: 180 }
            ];
            let html = '';
            categories.forEach((cat, idx) => {
                html += `
                    <tr>
                        ...
                        <td class="py-2 px-3 text-right font-bold text-[#ED6C02]">${cat.guests || 180}</td>
                    </tr>
                `;
            });
        ...
        if (tableNS) {
            const staff = item.nhanSu || [
                { title: 'Bác sĩ Khám Nội lưu động', count: 4 },
                { title: 'Điều dưỡng Lấy mẫu ngoại viện', count: 6 }
            ];
            let html = '';
            staff.forEach((st, idx) => {
                html += `
                    <tr>
                        ...
                        <td class="py-2 px-3 text-right font-bold text-[#ED6C02]">${st.count || 2}</td>
                    </tr>
                `;
            });
