/**
 * MWK - CHỈNH SỬA LỊCH TẠI VIỆN | TAI-VIEN MODULE EDIT JS LOGIC
 */
...
function customizeBreadcrumb() {
    ...
    breadcrumbContainer.innerHTML = `
        ...
        <i class="fa-solid fa-chevron-right text-[9px] text-[#9CA3AF]"></i>
        <span class="font-semibold text-[#1F2937]">Chỉnh sửa lịch tại viện</span>
    `;
}
...
function loadHeaderDataFromDraft() {
    ...
    const customerName = draft.customerName || draft.contactPerson || 'Công ty cổ phần ABC';
    const customerCode = draft.customerCode || 'KAD019962025';
    const estimatedCount = draft.estimatedCount || '250';
...
function loadSubScheduleData() {
    ...
    let list = draft.step2Data && draft.step2Data.taiVien ? draft.step2Data.taiVien : [];
    if (list.length === 0 && editId && window.MWKDataStore) {
        const master = MWKDataStore.getKskScheduleById(editId);
        if (master && master.step2Data && master.step2Data.taiVien) {
            list = master.step2Data.taiVien;
        }
    }
    ...
    if (item) {
        ...
        if (guestInput) guestInput.value = item.soLuongKhach || item.guests || 250;
...
function renderCategoryTable() {
    if (categoryRows.length === 0) {
        categoryRows = [
            { name: 'Siêu âm ổ bụng tổng quát', guests: 250, sang: true, chieu: true },
            { name: 'Khám Nội tổng quát', guests: 250, sang: true, chieu: true }
        ];
    }
        ...
        html += `
            <tr class="hover:bg-[#F8FAFC]">
                ...
                <td class="py-2 px-3 text-right">
                    <input type="number" min="0" value="${row.guests}" oninput="updateCategoryRow(${idx}, 'guests', this.value)" class="sys-input text-xs h-8 text-right font-bold text-[#27496D] w-24">
                </td>
                <td class="py-2 px-3 text-center">
                    <input type="checkbox" ${row.sang ? 'checked' : ''} onchange="updateCategoryRow(${idx}, 'sang', this.checked)" class="w-4 h-4 text-[#27496D] rounded-[2px]">
                </td>
                <td class="py-2 px-3 text-center">
                    <input type="checkbox" ${row.chieu ? 'checked' : ''} onchange="updateCategoryRow(${idx}, 'chieu', this.checked)" class="w-4 h-4 text-[#27496D] rounded-[2px]">
                </td>
                <td class="py-2 px-3 text-center">
...
function addCategoryRow() {
    markFormDirty();
    categoryRows.push({ name: 'Danh mục khám mới', guests: 250, sang: true, chieu: true });
    renderCategoryTable();
}
...
function renderStaffTable() {
    if (staffRows.length === 0) {
        staffRows = [
            { title: 'Bác sĩ Khám Nội', count: 4, sang: true, chieu: true },
            { title: 'Điều dưỡng Lấy máu', count: 4, sang: true, chieu: true }
        ];
    }
    ...
    staffRows.forEach((row, idx) => {
        html += `
            <tr class="hover:bg-[#F8FAFC]">
                ...
                <td class="py-2 px-3 text-right">
                    <input type="number" min="1" value="${row.count}" oninput="updateStaffRow(${idx}, 'count', this.value)" class="sys-input text-xs h-8 text-right font-bold text-[#27496D] w-20">
                </td>
                <td class="py-2 px-3 text-center">
                    <input type="checkbox" ${row.sang ? 'checked' : ''} onchange="updateStaffRow(${idx}, 'sang', this.checked)" class="w-4 h-4 text-[#27496D] rounded-[2px]">
                </td>
                <td class="py-2 px-3 text-center">
                    <input type="checkbox" ${row.chieu ? 'checked' : ''} onchange="updateStaffRow(${idx}, 'chieu', this.checked)" class="w-4 h-4 text-[#27496D] rounded-[2px]">
                </td>
                <td class="py-2 px-3 text-center">
...
function handleSaveEdit() {
    ...
    if (window.showToast) {
        window.showToast('Đã cập nhật lịch khám tại viện!', 'success');
    }
